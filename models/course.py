...
class Course:
    ...
    def __init__(self, id:int, title:str, code:str, unit:str, capacity:int):
        self.id=id
        self.title=title
        self.code=code
        self.unit=unit
    ...
    def assign_professor(self, professor):
        if self.professor is not None and self.professor.id==self.professor.id:
            raise ProfessorAlreadyAssignedException("این درس قبلا به این استاد اختصاص داده شده است.")
        if self.professor is not None and self.professor.id!=self.professor.id:
            raise ProfessorAlreadyAssignedException("این درس قبلا استاد داد")
